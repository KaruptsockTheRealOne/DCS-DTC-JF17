...
namespace DTC.Models.JF17.Waypoints
{
	public class Waypoint
	{
		...
		//, int elevation
		public Waypoint(int seq, string name, string latitude, string longitude)
		{
			Sequence = seq;
			Name = name;
			Latitude = latitude;
		...
		//, string elevation
		public static Waypoint FromStrings(string name, string coord)
		//, int.Parse(elevation)
		{
			var match = coordRegex.Match(coord);
			var wpt = new Waypoint(0, name, match.Groups[1].Value, match.Groups[2].Value);
			return wpt;
		}
